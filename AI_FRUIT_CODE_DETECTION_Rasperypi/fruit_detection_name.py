...
def map_code_to_name(csv_path: str | None = None):
    """
    Load Code->Name mapping from the CSV located next to this script.
    """
    if csv_path is None:
...
def one_shot_detect(crop):
    ...
    try:
        ...
        for x1, y1, x2, y2, canonical_name, cf in dets:
            ...
            display_name = code_to_name.get(canonical_name, canonical_name)
            line1 = f'{canonical_name} - {display_name}'
            line2 = f'Conf: {cf:.2f}'
